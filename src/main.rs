...
#[derive(Resource)]
struct CorruptionRateAdder {
    value: f32,
    timer: Timer,
}

impl Default for CorruptionRateAdder {
    fn default() -> Self {
        Self {
            value: 0.01,                                           // Initial value
            timer: Timer::from_seconds(5.0, TimerMode::Repeating), // Increase every 5 seconds
        }
    }
}
...
fn main() {
    App::new()
        ...
        .insert_resource(CorruptionRateAdder::default())
        .insert_resource(TileMap {
            tiles: HashMap::new(),
        })
        .insert_resource(PotentiallyCorruptedTiles { tiles: vec![] }) //{
        // .insert_resource(GameRng { rng: thread_rng() })
        ...
        // .add_systems(Update, collectables::spawn_collectable_notes)
